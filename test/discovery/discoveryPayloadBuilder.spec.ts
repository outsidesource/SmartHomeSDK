...
import { DiscoveryEndpointBuilder } from '../../src/discovery/endpointBuilder'
import { DiscoveryEndpoint, DiscoveryPayload, DisplayCategories, SemanticActionNames, SemanticStateNames } from '../../src/discovery/payload'
import { DiscoveryPayloadBuilder } from '../../src/discovery/payloadBuilder'
...
import { createSinonStubInstance, removeUndefinedProps, toStringWithLeadingZeros } from '../helpers'
...
describe('discovery payload builder', function () {
  ...
  describe('payload builder', function () {
    it('returns a comprehensive payload', function () {
    ...
    it('returns a payload with no endpoints when none are present', function () {
      const sut = new DiscoveryPayloadBuilder()
  ...
  describe('endpoint builder', function () {
    ...
    it('returns a minimal endpoint', function () {
    ...
    it('returns an endpoint with a unicode friendly name', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
    ...
    it('returns an endpoint with attributes when addAdditionalAttributes() is invoked multiple times', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')
      const expected = _.cloneDeep(minimalEndpoint)
      expected.additionalAttributes = {
        model: 'modelNumber',
    ...
    it('returns an endpoint with a Zigbee connection', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')
    ...
    it('returns an endpoint with a ZWave connection', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')
      const expectedEndpoint = _.cloneDeep(minimalEndpoint)
      expectedEndpoint.connections = [
    ...
    it('returns an endpoint with an unknown connection', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')
      const expectedEndpoint = _.cloneDeep(minimalEndpoint)
      expectedEndpoint.connections = [
    ...
    it('throws when given an invalid ZWave home id', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')

      expect(() => sut.withZWaveConnection('non-hexadecimal', '0x89')).to.throw('The ZWave home ID "non-hexadecimal" does not match the expected format.')
    })

    it('throws when given an invalid ZWave node id', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')

      expect(() => sut.withZWaveConnection('0x01234567', 'non-hexadecimal')).to.throw('The ZWave node ID "non-hexadecimal" does not match the expected format.')
    })

    it('throws when given an invalid unknown connection', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryPayloadBuilder)
      const sut = new DiscoveryEndpointBuilder(parentBuilderStub, 'endpointId', 'manufacturerName', 'description', 'friendly name')

      expect(() => sut.withUnknownConnection('connection with spaces')).to.throw('The unknown connection value "connection with spaces" does not match the expected format.')
    })
  })
  ...
  describe('additional attributes builder', function () {
    describe('returns valid attributes only X is provided', function () {
    ...
    it('returns undefined if no attributes are specified', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryEndpointBuilder)
      const sut = new AdditionalAttributesBuilder(parentBuilderStub)
  ...
  describe('capability builder', function () {
    it('returns a capability with verifications', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryEndpointBuilder)
    ...
    it('returns a capability with a friendly name', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryEndpointBuilder)
      const sut = new CapabilityBuilder(parentBuilderStub, 'interfaceName', 'version')
      const expectedCapability = {
        type: 'AlexaInterface',
      ...
      const actual = sut
        .withAssetResource('assetId')
        .getCapability()

      expect(removeUndefinedProps(actual)).to.deep.equal(expectedCapability)
    })

    it('returns a capability with additional properties when addProperties() is invoked multiple times', function () {
      const parentBuilderStub = createSinonStubInstance(DiscoveryEndpointBuilder)
      const sut = new CapabilityBuilder(parentBuilderStub, 'interfaceName', 'version')
      const expectedCapability = {
        type: 'AlexaInterface',
  ...
  describe('properties builder', function () {
    it('returns a properties with names only when no flags are specified', function () {
    ...
    it('returns a properties with flags only when no names are specified', function () {
      const parentBuilderStub = createSinonStubInstance(CapabilityBuilder)
      const sut = new PropertiesBuilder(parentBuilderStub)
      const expectedProperties = {
    ...
    it('returns undefined when neither names nor flags are specified', function () {
      const parentBuilderStub = createSinonStubInstance(CapabilityBuilder)
      const sut = new PropertiesBuilder(parentBuilderStub)
      const expectedProperties = {
        proactivelyReported: true,
      ...
      const actual = sut.getProperties()

      expect(removeUndefinedProps(actual)).to.be.undefined
    })
  })
  ...
  describe('semantic action builder', function () {
    it('returns a mapping when no payload is specified', function () {
      const parentBuilderStub = createSinonStubInstance(CapabilityBuilder)
