...
import { DiscoveryEndpoint, DiscoveryPayload, DisplayCategories, SemanticActionNames, SemanticStateNames } from '../src/directives/discovery/factory/DiscoveryPayload'
import { DiscoveryPayloadBuilder } from '../src/directives/discovery/factory/DiscoveryPayloadBuilder'
...
describe('discovery payload builder', function() {
  ...
  it('creates a comprehensive payload', function() {
    const builder = new DiscoveryPayloadBuilder()
    ...
    expect(builder.getPayload()).to.deep.equal(comprehensiveDiscoveryPayload)
  })
  it('creates a payload with no endpoints when none are present', function() {
    const builder = new DiscoveryPayloadBuilder()
    const payload = builder.getPayload()

    expect(payload.endpoints).to.be.empty
  })
  it('throws if too many endpoints are added', function() {
    ...
    expect(function(){ builder.getPayload() }).to.throw('The number of endpoints cannot exceed 300.')
  })
  it('throws if semantic action names are used more than once in a single endpoint', function() {
    ...
    expect(function(){ builder.getPayload() }).to.throw('Duplicate semantic action names found for the following: [{"action":"Alexa.Actions.Open","locations":[{"endpoint":"endpointId","capability":"capability0"},{"endpoint":"endpointId","capability":"capability1","instance":"instanceName"}]}]')
  })
  it('throws if semantic action names are used more than once across multiple endpoints', function() {
    ...
    expect(function(){ builder.getPayload() }).to.throw('Duplicate semantic action names found for the following: [{"action":"Alexa.Actions.Lower","locations":[{"endpoint":"endpointId0","capability":"capabilityName","instance":"instanceName"},{"endpoint":"endpointId1","capability":"capabilityName","instance":"instanceName"}]}]')
  })
