...
export interface SmartHomeSkillBuilder {
  addRequestHandler(matcher: ((input: HandlerInput<ResponseBuilder>) => Promise<boolean> | boolean) | PayloadSignature, executor: (input: HandlerInput<ResponseBuilder>) => Promise<Response<ResponsePayload>> | Response<ResponsePayload>): this
  addRequestHandlers(...requestHandlers: Array<RequestHandler<HandlerInput<ResponseBuilder>, Response<ResponsePayload>>>): this
  addRequestInterceptors(...executors: Array<SmartHomeSkillRequestInterceptor | ((input: HandlerInput<ResponseBuilder>) => Promise<void> | void)>): this
  addResponseInterceptors(...executors: Array<SmartHomeSkillResponseInterceptor | ((input: HandlerInput<ResponseBuilder>, response?: Response<ResponsePayload>) => Promise<void> | void)>): this
  addErrorHandler(matcher: (input: HandlerInput<ResponseBuilder>, error: Error) => Promise<boolean> | boolean, executor: (input: HandlerInput<ResponseBuilder>, error: Error) => Promise<Response<ResponsePayload>> | Response<ResponsePayload>): this
  ...
  withHandlerInputFactories(...handlerInputFactories: Array<HandlerInputFactory<ResponseBuilder>>): this
